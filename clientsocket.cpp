    ...
    QTcpSocket(parent), request_(request), response_(response), parser_(parser)
{
    ...
    connect(parser, SIGNAL(readyRead(QByteArray)), this, SLOT(readClient(QByteArray)));
    connect(this, SIGNAL(disconnected()), this, SLOT(deleteLater()));
}

void ClientSocket::readClient(QByteArray snippet)
{
    request_->requestReady(snippet);
}
